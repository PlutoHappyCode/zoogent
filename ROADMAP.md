...
| 日期 | 更新 | 影响面 |
| --- | --- | --- |
| 2026-08-18 | **模型可见文字全面英文化**：system prompt 模板（lean+classic）、5 个记忆工具 schema、全部工具返回串、5 个技能文件的 52 处 description + ~84 处返回串——给模型读的全部改成精简英文（token 更省、指令遵循更稳）；给用户看的（卡片回复、斜杠命令、故障提示）保持中文；"Reply in Chinese" 规则保留在 prompt 里 | core/personas.py、core/memory.py、core/engine.py、AgentsHome/skills/*.py ×5 |
| 2026-08-18 | **响应提速四件套**（诊断报告驱动）：① memory_write_batch 批量记忆写入——多文件更新（journal/看板/good）从 4-5 轮 API 往返压到 1 轮，纪律第 5 条强制攒批 ② 陈旧 tool 结果自动压缩——2 轮前的 tool content 截到 200 字符（实测占会话 41%，管家狗 15.4k tokens/轮的膨胀主源）③ 三阶段表情后台异步化——敲键盘/撒花切换不阻塞回复链路 ④ API 计时埋点——每轮记耗时/输入字符/输出 tokens 到日志，footer 数据可溯源 | core/memory.py、core/personas.py、core/engine.py、channels/feishu/channel.py、agent.json（housekeeper 摘掉 stock） |
| 2026-08-17 | **飞书文档链路全通**：① lark-cli 升级 v2 接口适配（docs +create/--doc-format markdown、+update --command append、fetch 显式 markdown、base +record-list --json、members create --yes）② bot 身份自助授权闭环——feishu_auth_start/status 两工具，agent 遇 not_configured 自动用 agent.json 里的 app 凭证初始化（--app-secret-stdin 非交互），不再弹人工授权链接；状态存挂载卷 /root/.lark-cli 重启不丢 ③ 建文档后自动按 owner_open_ids 分享给主人 ④ Dockerfile 固化 lark-cli 1.0.88（重建容器不回退） | AgentsHome/skills/feishuDocs.py、Dockerfile（新增） |
| 2026-08-17 | **模型与稳定性修复**：① 模型兜底去硬编码 default——改名 kimi 后 agent_model_name/get_client/model_config 全线崩溃，改动态兜底（优先 default，没有则用第一个非 embed 模型）② 孤儿 tool_calls 消毒——磁盘会话恢复时自动补缺失 tool 响应占位符（kimi 严格校验 tool 配对，管家狗从 qwen 换 kimi 后全线 400 的根因）③ 工具调用熔断——同工具同参数连调 3 次判定死循环强制终止（授权流程曾空转烧 40 万 token）④ NAS 配置统一只读 /data/zoogent/agent.json（AGENT_CONFIG），修掉/ app/agent.json 与数据目录两份配置不一致的坑 | core/models.py、core/memory.py、core/engine.py、core/config.py、NAS .env |
| 2026-08-16 | **全员格文档精简**：8 个 agent（除 askme）的 soul/rules/lean/memory 全面瘦身 57-92%，标准化格式；删除冗余 archive/daily-logs 重复目录；同步 NAS | AgentsHome/*/（soul.md、rules.md、lean.md、memory/） |
